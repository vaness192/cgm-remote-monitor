...
function init (ctx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function concatConsoleError (valueParts) {
      if (prop.lastSuggested && prop.lastSuggested.consoleError) {
        const consoleErrors = prop.lastSuggested.consoleError.join('<br>');
        valueParts.push(` <b>Dados AutoISF:</b> ${consoleErrors}`);
      }
      return valueParts;
    }
    ...
    function addSuggestion () {
      if (prop.lastSuggested) {
        ...
        valueParts = concatIOB(valueParts);
        valueParts = concatConsoleError(valueParts);
